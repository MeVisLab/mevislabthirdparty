...
class ConanRecipe(ConanFile):
    ...
    def source(self):
        ...
        patch(
            self, patch_file="patches/001-fix_windows_debug_build.patch",
            patch_type="bugfix",
            patch_description="fix debug build linker issue on MSVC",
            patch_source="https://gitlab.kitware.com/vtk/vtk/-/issues/19166"
        )
        patch(
            self, patch_file="patches/002-blit_error_checking.patch",
            patch_type="bugfix",
            patch_description="""fix OpenGL error not being detected for the correct call
Issue submitted: https://gitlab.kitware.com/vtk/vtk/-/issues/19254""",
        )
        patch(
            self, patch_file="patches/003-make_depth_buffer_configurable.patch",
...
Issue submitted: https://gitlab.kitware.com/vtk/vtk/-/issues/19255""",
        ...
        patch(
            self, patch_file="patches/004-allow_to_blit_back_depth.patch",
            patch_type="enhancement",
...
Issue submitted: https://gitlab.kitware.com/vtk/vtk/-/issues/19256""",
        ...
        replace_in_file(self, self.source_path / "ThirdParty" / "glew" / "CMakeLists.txt",
                        'set(VTK_GLEW_SHARED "${vtkglew_is_shared}")', 'set(VTK_GLEW_SHARED 1)')
    ...
    def generate(self):
        ...
        if self.settings.os == "Windows":
            ...
            tc.variables["PYTHON_LIBRARY"] = (
                    python_dependency.cpp_info.libdirs[0].replace('\\', '/')
                    + f"/python{py_version.major}{py_version.minor}{build_suffix}.lib"
            )
            ...
            tc.variables["PYTHON_EXECUTABLE"] = (
                    python_dependency.package_folder.replace('\\', '/') + f"/python3{build_suffix}.exe"
            )
        else:
    ...
    def package(self):
        copy(self, "Copyright.txt", src=self.source_path, dst=self.package_path / "licenses")
        cmake = CMake(self)
        cmake.install()
        ...
        if self.settings.build_type == 'Debug':
            ...
            if self.settings.os == "Windows":
                with chdir(self, self.package_path / "Lib/site-packages/vtkmodules"):
            ...
            if self.settings.os == "Linux":
                ...
                with chdir(self, self.package_path / f"lib/python{py_version_str}/site-packages/vtkmodules"):
                    for entry in glob.glob('*_d.so'):
                        rename(self, entry, entry[:-5]+'.so')

        if self.settings.os == "Windows":
            for dll in (self.package_path / "bin").glob("*.dll"):
                copy(self, f"*/{dll.stem}.pdb", src=self.build_path, dst=self.package_path / "bin", keep_path=False)
            for exe in (self.package_path / "bin").glob("*.exe"):
                copy(self, f"*/{exe.stem}.pdb", src=self.build_path, dst=self.package_path / "bin", keep_path=False)
    ...
    def package_info(self):
        ...
        self.cpp_info.defines.append(
            'VTK_RENDERING_BACKEND_OPENGL' if self.settings.os == "Macos" else 'VTK_RENDERING_BACKEND_OPENGL2'
        )
