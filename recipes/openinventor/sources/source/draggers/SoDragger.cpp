...
//
bool
SoDragger::mouseButtonPressed(const SoEvent *ev)
{
  return ( SO_MOUSE_PRESS_EVENT(ev, BUTTON1) && enable.getValue() );
}
...
//
bool
SoDragger::mouseButtonReleased(const SoEvent *ev)
{
  return ( SO_MOUSE_RELEASE_EVENT(ev, BUTTON1) && enable.getValue());
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    return (bool) (lengthSquared >= minGesture*minGesture);
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    if ( somethingClose != NULL && somethingClose->containsNode(this) ) {
        ...
        bool oldBool = SoBaseKit::isSearchingChildren();
        ...
        for ( int i = 0; i < tempPathToThis->getLength(); i++ ) {
            ...
            if ( !pathNode  || !pathNode->getChildren() )
                ...
                tempPathNumKidsHack->append( 
                            (void *) (size_t) pathNode->getChildren()->getLength() );
        }
    }
    ...
    for(int numFmTail = 0; numFmTail < tempPathToThis->getLength();numFmTail++){
        ...
        if (numFmTail == 0) {
            ...
            if ( aList.find( nodeInPath, SoNotRec::PARENT ) == -1 ) {
                ...
                int numKidsBefore = (int) ((size_t) (*tempPathNumKidsHack)[numFmHead]);
                ...
                if (   (numKidsNow != numKidsBefore)
                    ...
                    if ( numKidsNow != numKidsBefore ) {
                        ...
                        tempPathNumKidsHack->insert( (void *) (size_t) numKidsNow, numFmHead);
...
////////////////////////////////////////////////////////////////////////
{
    ...
    // identity...
        bool childIdent = ( childMotion == SbMatrix::identity() );
...
{
    bool oldVal = valueChangedCallbacksEnabled;
                                                     ...
                                                     SoDragger *childDragger)
{
    ...
    parent->setActiveChildDragger( childDragger );
        ...
        // motion of the childDragger into our own motion matrix.
            ...
            bool saveEnabled = parent->enableValueChangedCallbacks( FALSE );
...
{
    bool    isCnvIdent = (conversion == NULL 
                          || (*conversion) == SbMatrix::identity());
    bool    isMtxIdent = (mtx        == SbMatrix::identity());
