...
SoTabPlaneDragger::SoTabPlaneDragger()
{
    ...
    if (SO_KIT_IS_FIRST_INSTANCE())
        readDefaultParts("tabPlaneDragger.iv", (const char*)geomBuffer, sizeof(geomBuffer));
...
SoTabPlaneDragger::reallyAdjustScaleTabSize(SoGLRenderAction *action)
{
    ...
    const float maxTabSize = 0.4f;
    for (int i=0;i<2;++i) {
        if ((screenLengths[i] < 10e-10) || (localTabSize[i] > maxTabSize)) {
            // check against division by (almost) zero and
            // don't let tab size get greater than the drag plane
            localTabSize[i] = maxTabSize;
        }
    }
