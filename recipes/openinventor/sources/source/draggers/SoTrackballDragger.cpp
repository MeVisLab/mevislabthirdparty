...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (SO_KIT_IS_FIRST_INSTANCE())
        readDefaultParts("trackballDragger.iv", (const char*)geomBuffer, sizeof(geomBuffer) );
...
////////////////////////////////////////////////////////////////////////
{
    ...
    switch( currentState ) {
        ...
        case FREE_ROTATE:
            ...
            initSphereProjectorInternal(SbVec3f(0,0,0), rad, startWorkSpaceHitPt, workSpaceToWorld);
            // allow a derived class to have its own projector
            SbProjector*  proj = initSphereProjector(SbVec3f(0,0,0), rad, startWorkSpaceHitPt, workSpaceToWorld);
            ...
            // Refactoring (MMS Bug: 57374)
            ptOnProjector = proj->project(getNormalizedLocaterPosition());
...
//    Result of refactoring (MMS Bug: 57374)
//
void SoTrackballDragger::initSphereProjectorInternal(SbVec3f center, float radius, SbVec3f startWorkSpaceHitPt, SbMatrix workingSpace)
{
  // This code block was implemented in dragStart() originally

  sphereProj->setSphere( SbSphere( center, radius ));
...
//    Result of refactoring (MMS Bug: 57374)
//
void SoTrackballDragger::initStripeProjectorInternal(SbLine axis, float radius, SbVec3f startWorkSpaceHitPt, SbMatrix workingSpace)
{
  // This code block was implemented in dragStart() originally

  stripeProj->setCylinder( SbCylinder(axis, radius) );
...
//
void 
SoTrackballDragger::getSphereDeltaRotation(SbRotation &deltaRot, SbVec3f &newHitPt, SbVec3f prevHitPt, SbMatrix workingSpace)
{
  ...
  // Bug 60708 - prevent swallowing of small changes:
  // if a zero rotation was computed, because the current hit point is very close to the previous one
  // we do not overwrite prevHitPt. To prevent this here, we simply set the newHitPt to the previous one
  if (deltaRot.getAngle() < FLT_EPSILON) {
    newHitPt = prevHitPt;
  }
}
////////////////////////////////////////////////////////////////////////
...
//
void 
SoTrackballDragger::getStripeDeltaRotation(SbRotation &deltaRot, SbVec3f &newHitPt, SbVec3f prevHitPt, SbMatrix workingSpace)
{
  // This code block was implemented in rotateDrag() originally
...
SbProjector* SoTrackballDragger::initSphereProjector( SbVec3f /*center*/, float /*radius*/, SbVec3f /*startWorkSpaceHitPt*/, SbMatrix /*workingSpace*/ )
{
  return sphereProj;
}

SbProjector* SoTrackballDragger::initStripeProjector( SbLine /*axis*/, float /*radius*/, SbVec3f /*startWorkSpaceHitPt*/, SbMatrix /*workingSpace*/ )
{
  return stripeProj;
}
