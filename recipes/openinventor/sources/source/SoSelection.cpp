...
////////////////////////////////////////////////////////////////////////
{
    ...
    SO_NODE_ADD_FIELD(selectOnlyOnClick, (FALSE));
...
{    
    ...
    if (selectOnlyOnClick.getValue() && !abortSelection) {
      if (event->isOfType(SoLocation2Event::getClassTypeId())) {
        // dragging more than two pixels from the mouse down position aborts the selection
        SbVec2s diff = mouseDownPosition - event->getPosition();
        if (ABS(diff[0]) > 3 || ABS(diff[1]) > 3) { // this should depend on the resolution of the screen
          abortSelection = TRUE;
        }
      }
      else if (event->isOfType(SoMouseWheelEvent::getClassTypeId())) {
        // turning the mouse wheel also aborts the selection
        abortSelection = TRUE;
      }
    }
    ...
    if (be->getState() == SoButtonEvent::DOWN) {
      mouseDownPosition = be->getPosition();
      abortSelection = FALSE;
    }
    ...
    if ((pickMatching && (be->getState() == SoButtonEvent::DOWN)) ||
        ...
        if (be->getState() == SoButtonEvent::DOWN) {
            ...
            mouseDownPickPath = copyFromThis(getPickedPath(pickedPoint));
        ...
        // Else button release.
            ...
            // traversal, and the selection wasn't aborted, handle it here.
            if (! action->isHandled() && !abortSelection) {
                ...
                SoPath *pickPath = getPickedPath(pickedPoint);
                if (pickPath == NULL) {
...
SoPath*
SoSelection::getPickedPath(const SoPickedPoint* pickedPoint)
{
...
//
void
SoSelection::invokeSelectionPolicy(SoPath *path, bool shiftDown)
//
////////////////////////////////////////////////////////////////////////
{
