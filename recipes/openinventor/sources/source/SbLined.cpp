 ...
 *
 ...
 |      SbLine
 |
 |   Author(s)          : originally by Nick Thompson, Howard Look
...
//////////////////////////////////////////////////////////////////////////////
//
// Line class with double values.
//
//////////////////////////////////////////////////////////////////////////////

// construct a line given 2 points on the line
SbLined::SbLined(const SbVec3d &p0, const SbVec3d &p1)
{
    setValue(p0, p1);
}
...
void
SbLined::setValue(const SbVec3d &p0, const SbVec3d &p1)
{
    pos = p0;
    dir = p1 - p0;
    dir.normalize();
...
bool
SbLined::getClosestPoints( const SbLined &line2,
                                 SbVec3d &ptOnThis,
                                 SbVec3d &ptOnLine2 ) const
{
    double   s, t, A, B, C, D, E, F, denom;
    SbVec3d  pos2 = line2.getPosition();
    SbVec3d  dir2 = line2.getDirection();
...
SbVec3d
SbLined::getClosestPoint(const SbVec3d &point) const
//
////////////////////////////////////////////////////////////////////////
{
    ...
    // but |d| = 1, so
    double length = p.dot(dir);

    // vector coincident with this line
    SbVec3d proj = dir;
    proj *= length;

    SbVec3d result = pos + proj;
    return result;
}    
...
bool
SbLined::intersect(const SbBox3d &box, SbVec3d &enter, SbVec3d &exit) const
//
////////////////////////////////////////////////////////////////////////
{
    if (box.isEmpty())
    ...
    const SbVec3d       &pos = getPosition(), &dir = getDirection();
    const SbVec3d       &max = box.getMax(), &min = box.getMin();
    SbVec3d             points[8], inter, bary;
    SbPlaned            plane;
    int                 i, v0, v1, v2;
    ...
    //
    double   t = (box.getCenter() - pos).dot(dir);
    SbVec3d  diff(pos + t * dir - box.getCenter());
    double   dist2 = diff.dot(diff);
    double   radi2 = (max - min).dot(max - min) * 0.25;
...
bool
SbLined::intersect(double angle, const SbBox3d &box) const
//
////////////////////////////////////////////////////////////////////////
{
    if (box.isEmpty())
    ...
    if (angle < 0.0)
        ...
        for(i = 0; i < 8; i++) {
            SbVec3d point(i & 01 ? min[0] : max[0],
                          i & 02 ? min[1] : max[1],
                          i & 04 ? min[2] : max[2]);
            // how far is point from line origin??
            SbVec3d     diff(point - getPosition());
            double      thisFuzz = sqrt(diff.dot(diff)) * tanA;

            if (thisFuzz > fuzz)
                fuzz = double(thisFuzz);
        }
    }

    SbBox3d fuzzBox = box;

    fuzzBox.extendBy(SbVec3d(min[0] - fuzz, min[1] - fuzz, min[2] - fuzz));
    fuzzBox.extendBy(SbVec3d(max[0] + fuzz, max[1] + fuzz, max[2] + fuzz));

    SbVec3d scratch1, scratch2;
...
bool
SbLined::intersect(
    double pickAngle,                    // The angle which makes the cone
    const SbVec3d &point) const         // The point to interesect.
//
////////////////////////////////////////////////////////////////////////
{
    double      t, d;
    ...
    if(t > 0) {
        d = double(sqrt(diff.dot(diff)) - t*t);
        if (pickAngle < 0.0)
            return (d < -pickAngle);
...
SbLined::intersect(
   double pickAngle,             // The angle which makes the cone
   const SbVec3d &v0,           // One endpoint of the line segment
   const SbVec3d &v1,           // The other endpoint of the line segment
   SbVec3d &intersection) const // The intersection point
//
////////////////////////////////////////////////////////////////////////
{
    SbVec3d     ptOnLine;
    SbLined     inputLine(v0, v1);
    double      distance;
...
bool
SbLined::intersect(const SbVec3d &v0, const SbVec3d &v1, const SbVec3d &v2,
                  SbVec3d &intersection,
                  SbVec3d &barycentric, bool &front) const
//
////////////////////////////////////////////////////////////////////////
{
    //////////////////////////////////////////////////////////////////
...
#define EPSILON 1e-10
    ...
    SbVec3d     v01 = v1 - v0;
    SbVec3d     v12 = v2 - v1;
    SbVec3d     norm = v12.cross(v01);  // Un-normalized normal
    ...
    //
    double d = getDirection().dot(norm);
    if (d < EPSILON && d > -EPSILON)
        return FALSE;                   // Line is parallel to plane
    double t = norm.dot(v0 - getPosition()) / d;
    ...
    //
    double      xAbs = norm[0] < 0.0 ? -norm[0] : norm[0];
    double      yAbs = norm[1] < 0.0 ? -norm[1] : norm[1];
    double      zAbs = norm[2] < 0.0 ? -norm[2] : norm[2];
    int         axis0, axis1;
    ...
    //     third computation.
    //
    double intersection0 = getPosition()[axis0] + t * getDirection()[axis0];
    double intersection1 = getPosition()[axis1] + t * getDirection()[axis1];

    SbVec2d     diff0, diff1, diff2;
    bool        isInter = FALSE;
    double      alpha, beta;
