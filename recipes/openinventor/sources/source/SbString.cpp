...
{
    int newSize = static_cast<int>(strlen(string) + bySize + 1);
...
void
SbString::makeEmpty(bool freeOld)
{
    if (string != staticStorage) {
        if (freeOld)
            delete [] string;
...
{
    int size = static_cast<int>((str == NULL ? 0 : strlen(str)) + 1);
...
{
    if (str != NULL) {
        expand(static_cast<int>(strlen(str)));
        strcat(string, str);
    }
    return *this;
}
