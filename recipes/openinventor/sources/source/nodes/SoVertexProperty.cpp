...
// Using the indirections below avoids the standard call convention on Windows which can lead to crashes.
void SoVertex3fv(const char* data) { glVertex3fv(reinterpret_cast<const GLfloat*>(data)); }
void SoVertex4fv(const char* data) { glVertex4fv(reinterpret_cast<const GLfloat*>(data)); }
void SoNormal3fv(const char* data) { glNormal3fv(reinterpret_cast<const GLfloat*>(data)); }
void SoTexCoord2fv(const char* data) { glTexCoord2fv(reinterpret_cast<const GLfloat*>(data)); }
void SoTexCoord4fv(const char* data) { glTexCoord4fv(reinterpret_cast<const GLfloat*>(data)); }
...
void SoVertexProperty::putVBOsIntoState( SoState* state )
{
  if (state->isElementEnabled(SoGLVBOElement::getClassStackIndex())) {
    if (vertex.getNum() > 0) {
      SoGLVBOElement::updateVBO(state, SoGLVBOElement::VERTEX_VBO, &_vertexVBO,
        vertex.getNum() * sizeof(SbVec3f), vertex.getValues(0), getNodeId());
    }
    if (normal.getNum() > 0) {
      SoGLVBOElement::updateVBO(state, SoGLVBOElement::NORMAL_VBO, &_normalVBO,
        normal.getNum() * sizeof(SbVec3f), normal.getValues(0), getNodeId());
    }
    if (orderedRGBA.getNum() > 0
      && ! SoOverrideElement::getDiffuseColorOverride(state)) {
        // update the VBO, no data is passed since that is done by SoColorPacker later on
        SoGLVBOElement::updateVBO(state, SoGLVBOElement::COLOR_VBO, &_colorVBO);
    }
    if ( texCoord.getNum() > 0) {
      SoGLVBOElement::updateVBO(state, SoGLVBOElement::TEXCOORD_VBO, &_texCoordVBO,
        texCoord.getNum() * sizeof(SbVec2f), texCoord.getValues(0), getNodeId());
    }
  }
}

void vp_glColor4ubv(const char *v)
{
    GLubyte _v[4];
                                                 ...
                                                 SoState *state)
{
    // Diffuse colors:
    transpIsInVP = FALSE;
    bool colorOverride = ((SoOverrideElement::getDiffuseColorOverride(state))||
