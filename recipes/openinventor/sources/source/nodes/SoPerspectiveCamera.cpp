...
////////////////////////////////////////////////////////////////////////
{
    ...
    if (viewportMapping.getValue()!=ADJUST_CAMERA &&
      viewportMapping.getValue()!=LEAVE_ALONE &&
      aspect < 1.0) {
        hOverD *= aspect;
    }
    float radius = bSphere.getRadius();
    distToCenter = radius / hOverD;  //   h / (h/d) gives us d

    // we calculated h and d, but we need the hypotenuse
    distToCenter = sqrt(radius * radius + distToCenter*distToCenter);
