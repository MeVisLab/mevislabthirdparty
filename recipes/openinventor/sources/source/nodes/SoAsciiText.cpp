...
public:
  MyFontOutline() = default;
  MyFontOutline(MyFontOutline &&) noexcept = default;
  MyFontOutline(const MyFontOutline &) = default;
  MyFontOutline &operator=(MyFontOutline &&) noexcept = default;
  MyFontOutline &operator=(const MyFontOutline &) = default;

  // Constructor, takes a pointer to the font-library outline
  // structure and the font's size:
  MyFontOutline(const FL::Outline *outline, float fontSize);

  // Query routines:
  int getNumOutlines() const
  {
    return numOutlines;
  }
  int getNumVerts(int i) const
  {
    return numVerts[i];
  }
  SbVec2f &getVertex(int i, int j) &
  {
    return verts[i][j];
  }

  const SbVec2f &getVertex(int i, int j) const &
  {
    return verts[i][j];
  }

  SbVec2f getCharAdvance() const
  {
    return charAdvance;
  }
...
public:
  ...
  static MyOutlineFontCache *getFont(SoState *, bool forRender);
  ...
  void renderFront(const SbString &string, float width, gluTESSELATOR *tobj);
...
private:
  ...
  // outline.
  const MyFontOutline *getOutline(unsigned char c);

  // Some helper routines for generateSide:
  void figureSegmentNorms(SbVec2f *result, int nPoints, const SbVec2f *points, float cosCreaseAngle,
                          bool isClosed);
  void figureSegmentTexCoords(float *texCoords, int nPoints, const SbVec2f *points, bool isClosed);
  void fillBevel(SbVec3f *result, int nPoints, const SbVec2f *points, const SbVec2f &translation,
  ...
  // needed.
  std::array<MyFontOutline, 256> outlines;
...
bool MyOutlineFontCache::tesselationError = FALSE;
SbPList *MyOutlineFontCache::fonts = nullptr;
FL::Context *MyOutlineFontCache::context = nullptr;

// Static stuff is used while generating primitives:
SoAsciiText *SoAsciiText::currentGeneratingNode = nullptr;
...
SoAction *SoAsciiText::genAction = nullptr;
bool SoAsciiText::genBack = FALSE;
bool SoAsciiText::genTexCoord = TRUE;
const SoTextureCoordinateElement *SoAsciiText::tce = nullptr;
...
void SoAsciiText::GLRender(SoGLRenderAction *action)
//
////////////////////////////////////////////////////////////////////////
{
  ...
  bool materialPerPart = (mbe == SoMaterialBindingElement::PER_PART_INDEXED ||
  ...
  if (tobj == nullptr)
  {
    ...
    gluTessCallback(tobj, (GLenum)GLU_ERROR, (OPENGL_CALLBACKFUNC)MyOutlineFontCache::errorCB);
  }
...
void SoAsciiText::rayPick(SoRayPickAction *action)
//
////////////////////////////////////////////////////////////////////////
{
...
void SoAsciiText::computeBBox(SoAction *action, SbBox3f &box, SbVec3f &center)
//
////////////////////////////////////////////////////////////////////////
{
...
void SoAsciiText::generatePrimitives(SoAction *action)
//
////////////////////////////////////////////////////////////////////////
{
  SoState *state = action->getState();
...
bool SoAsciiText::setupFontCache(SoState *state, bool forRender)
//
////////////////////////////////////////////////////////////////////////
{
...
void SoAsciiText::getFrontBBox(SbBox2f &result)
//
////////////////////////////////////////////////////////////////////////
{
  SbBox2f charBBox;
  ...
  for (line = 0; line < string.getNum(); line++)
  {
    ...
    if (w > 0)
    {
      ...
      result.setBounds(charPosition[0], min[1], charPosition[0] + w, max[1]);
    }
  }
}
...
SbVec2f SoAsciiText::getStringOffset(int line, float width)
//
////////////////////////////////////////////////////////////////////////
{
  SbVec2f result(0, 0);
...
void SoAsciiText::renderFront(SoGLRenderAction *, const SbString &string, float width,
                              gluTESSELATOR *tobj)
//
////////////////////////////////////////////////////////////////////////
{
...
SoDetail *SoAsciiText::createTriangleDetail(SoRayPickAction *, const SoPrimitiveVertex *v1,
...
void SoAsciiText::generateFront(const SbString &string, float width)
//
////////////////////////////////////////////////////////////////////////
{
  static gluTESSELATOR *tobj = nullptr;
...
void SoAsciiText::beginCB(GLenum primType)
//
////////////////////////////////////////////////////////////////////////
{
...
////////////////////////////////////////////////////////////////////////
{
  ...
  if (genWhichVertex == 0)
  {
    ...
    if (genBack)
    {
      t3->invokeTriangleCallbacks(genAction, genPrimVerts[2], genPrimVerts[1], genPrimVerts[0]);
    }
    else
    {
      t3->invokeTriangleCallbacks(genAction, genPrimVerts[0], genPrimVerts[1], genPrimVerts[2]);
    }
...
MyOutlineFontCache *MyOutlineFontCache::getFont(SoState *state, bool forRender)
//
////////////////////////////////////////////////////////////////////////
{
  if (fonts == nullptr)
  {
    ...
    {
#ifdef DEBUG
      SoDebugError::post("SoAsciiText::getFont", "FL::Context returned nullptr");
#endif
      return nullptr;
    }
  ...
  MyOutlineFontCache *result = nullptr;
  for (int i = 0; i < fonts->getLength() && result == nullptr; i++)
  {
    ...
    if (forRender ? c->isRenderValid(state) : c->isValid(state))
    {
      ...
      if (FL::currentFont() != result->fontId)
      {
        FL::setCurrentFont(result->fontId);
      }
    }
  }
  // If none match:
...
bool MyOutlineFontCache::isValid(const SoState *state) const
//
////////////////////////////////////////////////////////////////////////
{
  ...
  if (result)
  {
    ...
    {
      FL::setCurrentContext(context);
      FL::setCurrentFont(fontId);
    }
    else if (FL::currentFont() != fontId)
    {
      FL::setCurrentFont(fontId);
    }
  }
...
bool MyOutlineFontCache::isRenderValid(SoState *state) const
//
////////////////////////////////////////////////////////////////////////
{
  if (!isValid(state))
  ...
  if (frontList && frontList->getContext() != SoGLCacheContextElement::get(state))
    return FALSE;
...
////////////////////////////////////////////////////////////////////////
{
  ...
  addElement(state->getConstElement(SoComplexityElement::getClassStackIndex()));
  addElement(state->getConstElement(SoComplexityTypeElement::getClassStackIndex()));
  ...
  case SoComplexityTypeElement::OBJECT_SPACE:
  {
    ...
    const float ONE = 1;
    if (complexity > 0.5)
      uems = (2.0 - complexity * 2.0) * (HALF - ONE) + ONE;
    else
      uems = (1.0 - complexity * 2.0) * (ZERO - HALF) + HALF;
  }
  break;
  ...
  case SoComplexityTypeElement::SCREEN_SPACE:
  {
    ...
    float maxSize = (rectSize[0] > rectSize[1] ? rectSize[0] : rectSize[1]);
    uems = 250.0 / (1.0 + 0.25 * maxSize * complexity * complexity);
    ...
    addElement(state->getConstElement(SoProjectionMatrixElement::getClassStackIndex()));
    addElement(state->getConstElement(SoViewingMatrixElement::getClassStackIndex()));
    addElement(state->getConstElement(SoModelMatrixElement::getClassStackIndex()));
  ...
  fontId = FL::Context::createFont(font.getString(), m, 0, nullptr);

  // If error creating font:
  if (fontId == FL::BadFontNumber)
  {
    // Try IV_DEFAULT_FONTNAME, unless we just did!
    if (font != SbName(IV_DEFAULT_FONTNAME))
    {
#ifdef DEBUG
      SoDebugError::post("SoAsciiText::getFont", "Couldn't find font %s, replacing with %s",
                         font.getString(), IV_DEFAULT_FONTNAME);
#endif
      fontId = FL::Context::createFont(IV_DEFAULT_FONTNAME, m, 0, nullptr);
    }
    if (fontId == FL::BadFontNumber)
    {
#ifdef DEBUG
      SoDebugError::post("SoAsciiText::getFont", "Couldn't find font %s!", IV_DEFAULT_FONTNAME);
#endif
      return;
    }
  }
  ...
  if (nProfileVerts > 1)
  {
    ...
    figureSegmentNorms(profileNorms, (int)nProfileVerts, profileVerts, cosCreaseAngle, FALSE);
    ...
    figureSegmentTexCoords(sTexCoords, (int)nProfileVerts, profileVerts, FALSE);
...
////////////////////////////////////////////////////////////////////////
{
  if (fontId)
  {
    ...
    for (int i = 0; i < fonts->getLength(); i++)
    {
      ...
      if (t != this && t->fontId == fontId)
        otherUsing = TRUE;
    }
    if (!otherUsing)
    {
      FL::Context::removeFont(fontId);
    }
...
void MyOutlineFontCache::destroy(SoState *)
//
////////////////////////////////////////////////////////////////////////
{
  // Pass in nullptr to freeList because this cache may be destroyed
...
float MyOutlineFontCache::getWidth(const SbString &string)
//
////////////////////////////////////////////////////////////////////////
{
  float total = 0.0;
  ...
  for (int i = 0; i < string.getLength(); i++)
  {
    const auto *outline = getOutline(chars[i]);
...
void MyOutlineFontCache::getCharBBox(const char c, SbBox2f &result)
//
////////////////////////////////////////////////////////////////////////
{
  result.makeEmpty();
  ...
  const auto *outline = getOutline(c);
...
const MyFontOutline *MyOutlineFontCache::getOutline(unsigned char c)
//
////////////////////////////////////////////////////////////////////////
{
  if (!fontId)
...
SbVec2f MyOutlineFontCache::getCharOffset(const char c)
//
////////////////////////////////////////////////////////////////////////
{
  if (!fontId)
...
void MyOutlineFontCache::generateFrontChar(const char c, gluTESSELATOR *tobj)
//
////////////////////////////////////////////////////////////////////////
{
  if (!fontId)
  ...
  const MyFontOutline *outline = getOutline(c);
  int i;
  for (i = 0; i < outline->getNumOutlines(); i++)
  {
    ...
    for (int j = 0; j < outline->getNumVerts(i); j++)
    {
      ...
      gluTessVertex(tobj, v, &const_cast<SbVec2f &>(t));
    }
  }
#ifdef GLU_VERSION_1_2
  gluTessEndContour(tobj);
...
void MyOutlineFontCache::setupToRenderFront(SoState *state)
//
////////////////////////////////////////////////////////////////////////
{
  ...
  {
    frontList = new SoGLDisplayList(state, SoGLDisplayList::DISPLAY_LIST, numChars);
    frontList->ref();
  }
  if (frontList)
  {
...
bool MyOutlineFontCache::hasFrontDisplayList(const char c, gluTESSELATOR *tobj)
//
////////////////////////////////////////////////////////////////////////
{
...
void MyOutlineFontCache::callFrontLists(const SbString &string, float off)
//
////////////////////////////////////////////////////////////////////////
{
...
void MyOutlineFontCache::renderFront(const SbString &string, float off, gluTESSELATOR *tobj)
//
////////////////////////////////////////////////////////////////////////
{
  const char *str = string.getString();
...
MyFontOutline::MyFontOutline(const FL::Outline *outline, float fontSize)
//
////////////////////////////////////////////////////////////////////////
{
  charAdvance = SbVec2f(outline->_xAdvance, outline->_yAdvance) * fontSize;
  numOutlines = outline->_outlineCount;
  if (numOutlines != 0)
  {
    numVerts.resize(numOutlines);
    verts.resize(numOutlines);
    for (int i = 0; i < numOutlines; i++)
    {
      numVerts[i] = outline->_vertexCounts[i];
      if (numVerts[i] != 0)
      {
        verts[i].resize(numVerts[i]);
        for (int j = 0; j < numVerts[i]; j++)
        {
          verts[i][j] = SbVec2f(outline->_vertex[i][j].x, outline->_vertex[i][j].y) * fontSize;
        }
      }
    }
  }
...
void MyOutlineFontCache::figureSegmentNorms(SbVec2f *norms, int nPoints, const SbVec2f *points,
                                            float cosCreaseAngle, bool isClosed)
//
////////////////////////////////////////////////////////////////////////
{
  int nSegments;
...
void MyOutlineFontCache::figureSegmentTexCoords(float *texCoords, int nPoints,
...
void MyOutlineFontCache::fillBevel(SbVec3f *result, int nPoints, const SbVec2f *points,
                                   const SbVec2f &translation, const SbVec2f &n1, const SbVec2f &n2)
//
////////////////////////////////////////////////////////////////////////
{
...
void MyOutlineFontCache::fillBevelN(SbVec3f *result, int nNorms, const SbVec2f *norms,
                                    const SbVec2f &n)
//
////////////////////////////////////////////////////////////////////////
{
...
#ifdef DEBUG
void MyOutlineFontCache::errorCB(GLenum whichErr)
{
  SoDebugError::post("SoAsciiText::errorCB", "%s", gluErrorString(whichErr));
  tesselationError = TRUE;
}
#else  /* DEBUG */
void MyOutlineFontCache::errorCB(GLenum)
{
  tesselationError = TRUE;
}
#endif /* DEBUG */
