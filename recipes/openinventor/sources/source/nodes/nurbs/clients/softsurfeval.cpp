...
/*
 * softsurfaceeval.cpp - surface evaluator
 *
 * $Revision: 1.2 $
 */
...
#if !defined(__APPLE__) && (!defined(WIN32) || (_MSC_VER < 1300)) && (!defined(__GNUC__) || (__GNUC__ < 7))
inline REAL abs( REAL x ) { return (x < 0.0) ? -x : x; }
#endif
inline int equal( REAL x, REAL y ) { return (fabs(x-y) < 1.0E-6) ? 1 : 0; }
