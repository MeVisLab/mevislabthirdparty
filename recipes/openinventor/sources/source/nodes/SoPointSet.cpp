...
////////////////////////////////////////////////////////////////////////
{
  ...
  // called before state->pop() is called:
  {
        ...
        SoVBO::isVertexArrayRenderingAllowed())
    {
      ...
      SoGLLazyElement::drawArrays(state, GL_POINTS, curCoord, numPts);
      endVertexArrayRendering(action, useVBO);

#ifdef DEBUG
      if (SoDebug::GetEnv("IV_DEBUG_VBO_RENDERING")) {
        SoDebugError::postInfo("GLRenderInternal", "%s VA Rendering: %d Points", getTypeId().getName().getString(), numPts);
      }
#endif
      ...
      if (SoDebug::GetEnv("IV_DEBUG_LEGACY_RENDERING")) {
        SoDebugError::postInfo("GLRenderInternal", "%s Immediate Mode Rendering: %d Points", getTypeId().getName().getString(), numPts);
      }
#endif
