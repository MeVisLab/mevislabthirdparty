...
SoEXTENDER class INVENTOR_API SoBoundingBoxCache : public SoCache {
  ...
  public:
    ...
    bool                isCenterSet() const     { return centerSet; }
    ...
    bool                hasLinesOrPoints() const         { return hasLOrP; }

  private:
    SbXfBox3f           box;            //!< Bounding box
    SbBox3f             projectedBox;   //!< Box in local space
    bool                centerSet;      //!< If the center was set
    SbVec3f             center;         //!< Center point
    bool                hasLOrP;        //!< TRUE if contains lines or points
};
