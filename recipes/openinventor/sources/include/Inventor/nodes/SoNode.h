...
class INVENTOR_API SoNode : public SoFieldContainer {
  ...
  public:
    ...
    //! it copies the children as well. If \p copyConnections is
    ...
    //! SoSeparator)  that isolate their effects from the rest of the
    ...
    //! SoBase::setName().
    //! Returns the number of nodes with the given name, and adds to \p list pointers
    //! to those nodes.
  ...
  SoINTERNAL public:
    ...
    SbNodeIdType          getNodeId() const       { return uniqueId; }

    //! Returns the next available unique id
    static SbNodeIdType     getNextNodeId() { return nextUniqueId; }
  ...
  private:
    ...
    bool                override;       //!< TRUE if node overrides others
