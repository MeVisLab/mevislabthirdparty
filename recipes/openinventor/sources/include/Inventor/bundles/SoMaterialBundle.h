...
SoEXTENDER class INVENTOR_API SoMaterialBundle : public SoBundle {
  ...
  public:
    ...
    bool                isColorOnly() const     { return colorOnly; }

  private:
    bool                firstTime;      //!< TRUE if nothing sent yet
    int                 lastIndex;      //!< Last index reallySend()ed
    bool                colorOnly;      //!< TRUE if only base color sent
    bool                fastColor;      //!< TRUE if can send color for diffColor
    int                 numMaterials;   //!< Number of materials 
    bool                sendMultiple;   //!< indicates multiple diffuse are sent.

    //! Material component elements:
    const SoGLLazyElement               *lazyElt;

    //! Really sends stuff to GL
    void                reallySend(int index, bool isBetweenBeginEnd,
                                   bool avoidFastColor);

    //! Accesses and stores pointers to necessary elements
    void                accessElements(bool isBetweenBeginEnd,
                                       bool avoidFastColor);
};
