...
\ingroup Manips
SoSpotLightManip 
is the base class for all SoSpotLight nodes that have a built-in 
...
In this case, the interface edits the \b location , \b direction , and 
\b cutOffAngle  fields.
...
SoSpotLightManip (as when the user selects a light to be edited),
...
The SoSpotLightManip 
utilizes an SoSpotLightDragger to provide a 3D interface. 
However, the manipulator differs from the dragger; it lights other objects
in the scene because, as an SoSpotLight,
it alters the state. 
...
see the reference page for SoSpotLightDragger.  The interfaces of the
...
\b cutOffAngle  field of the <em>manipulator</em> will change by the same 
amount, thus changing the lighting of nodes which follow 
in the scene graph.
Similarly, if you set the \b cutOffAngle  field of the 
SoSpotLightManip, the manipulator will
...
<BR> First, traverses the dragger the way an SoGroup would. All draggers place themselves in space, but leave the current transformation unchanged when finished.  Then the SoSpotLightManip adds a spotlight to the state, just like its base class, SoSpotLight. 
\par
SoSearchAction
<BR> Searches just like an SoSpotLight.  Does not search the dragger, which is a hidden child. 
\par
SoWriteAction
<BR> Writes out just like an SoSpotLight. Does not write the dragger, which is a hidden child.   If you really need to write valuable information  about the dragger, such as customized geometry, you  can retrieve the dragger with the  getDragger() method and then write  it out separately. 
...
class INVENTOR_API SoSpotLightManip : public SoSpotLight
{
  ...
  public:
    ...
    //! The tail of the path must be an SoSpotLight node (or subclass thereof).
